...
description: "仅当用户显式输入 `$project-summary`，或 quality-reviewer 在 review 通过后明确要求项目上下文更新时使用。更新 AGENTS.md（project-context）和 CHANGELOG.md。"
---
...
按照下面的内容，在回复中维护简短 checklist，并把需要留存的状态写入对应文档。
...
3. 基于以下信息源更新各部分：
   - **PR diff**：实际的代码变更
   - **developer summaries**（来自 progress.md）：每个 task 做了什么
...
4. 逐部分检查是否需要更新：
   ...
   - **已知限制与技术债务**：从 developer summaries 和 progress.md 中提取（如 "临时用了 mock 数据"、"某个 task 跳过了"）
...
2. 基于 progress.md 中的 task 完成记录和 developer summaries，整理变更内容

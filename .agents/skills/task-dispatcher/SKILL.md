...
description: "仅当用户显式输入 `$task-dispatcher`，或明确要求使用 task-dispatcher workflow 时使用。读取 tasks.json，串行派发单任务 developer custom subagent，维护 progress.md。"
---
...
读取 implementation plan 与 tasks.json，按任务顺序逐个派发给 developer custom subagent 执行，验证每个任务的完成状态，维护全局 progress.md，最终在用户确认后创建 PR。
...
目的：按 tasks.json 数组顺序，逐个派发任务给 developer custom subagent，验证结果。
...
2. 按 `references/developer-prompt-template.md` 构建 developer custom subagent 的 prompt，且 prompt 必须显式包含 `$developer`，包含：
...
派发一个全新的 `developer` custom subagent。

要求：

- 每个 developer subagent 只做一个 task。
- developer subagent 必须遵循 $developer 的边界。
- developer subagent 不 commit。
- developer subagent 不选择下一个 task。
- developer subagent 不修改全局 implementation plan。

#### 4.3 检查结果

developer subagent 返回后，检查其 summary。
...
3. 如果质量检查通过：
   ...
   - 更新 progress.md：状态 -> `completed`，记录 developer subagent 的 summary

如果失败：

1. 更新 progress.md：记录本次失败原因。
2. 检查已尝试次数：
   - `< 3`：派发新的 `developer` custom subagent，prompt 中附加之前所有失败尝试的总结。

---
name: code-review
description: "仅当用户显式输入 `$code-review`，或 quality-reviewer 明确要求 code-review workflow 时使用。逐文件审查代码质量，输出结构化审查意见。"
---
...
1. 获取 PR 信息：
   ...
   - **否则**：审查当前 working tree diff
2. 如果审查 PR，使用 `gh pr view <PR> --json title,body,headRefName,baseRefName,files` 读取 PR 元信息，并使用 `gh pr diff <PR>` 获取完整 diff。
3. 如果审查当前 working tree，使用 `git diff` 和 `git status --short` 获取变更。

...
此模板用于 tech-designer workflow 的 Step 6，生成 `<topic>-implementation-plan.md`（纯 Markdown，面向人类审阅/ LLM agent 参考的公共上下文）。

## 使用约束

- **纯 Markdown**：不要把结构化任务塞进正文；任务列表落到 `tasks.json`
- **自包含**：后续开发时，人类开发者/LLM agent 只读本计划也能开工（不要求再读完整 product design 产物）

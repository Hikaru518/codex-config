---
name: tech-designer
description: "将 product design 阶段的 design doc 与 plans 文件夹下的文档转化为可串行执行的 implementation plan：包含 technical design、细粒度 tasks（每个 task 可由单个 LLM session 完成且可测试）、以及 tasks 的执行顺序与优先级。"
---
...
- `tasks.json` 必须遵循 `references/tasks-json-schema.md`，并按 json-lint skill 的要求确保可 parse。

...
3. 基于以下信息源更新各部分：
   - **PR diff**：实际的代码变更
   - **worker summaries**（来自 progress.md）：每个 task 做了什么
...
4. 逐部分检查是否需要更新：
   ...
   - **已知限制与技术债务**：从 worker summaries 和 progress.md 中提取（如 "临时用了 mock 数据"、"某个 task 跳过了"）
...
2. 基于 progress.md 中的 task 完成记录和 worker summaries，整理变更内容
